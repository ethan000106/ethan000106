...
- 🎓 **신소재공학(Advanced Materials Engineering) 전공**: 반도체 물성과 물리적 동작 메커니즘 이해 보유
- 🔬 **1.5년 학부 연구실 경험(접착 소재 연구)**: 데이터 분석력과 집요한 논리적 문제 해결 역량 확보
- 🔧 **RTL-to-Board 전 과정 경험**: 디지털 시스템을 RTL 설계 → 검증 → 보드 구동까지 직접 구현
- 🧩 **핵심 관심 분야**: FSM 설계, 프로토콜 인터페이스, 타이밍 이슈 디버깅
- 💡 **검증 지향적 태도**: "동작한다"에서 멈추지 않고, *왜* 동작하는지 파형(Waveform)으로 반드시 확인
